    ...
    public partial class TransactionViewModel
    {
        ...
        public async Task LoadRestaurantModePreference()
        {
            await ExecuteOperationSafelyAsync(async () =>
            {
                ...
                await System.Windows.Application.Current.Dispatcher.InvokeAsync(() =>
                {
                    ...
                    // Set barcode field visibility based on restaurant mode
                    IsBarcodeFieldVisible = isRestaurantMode;  // CORRECTED: Only visible when IN restaurant mode

                    // Force UI update
                    OnPropertyChanged(nameof(IsProductCardsVisible));
                    OnPropertyChanged(nameof(IsRestaurantMode));
                    OnPropertyChanged(nameof(IsBarcodeFieldVisible));

                    Debug.WriteLine($"Restaurant mode loaded from preferences: {isRestaurantMode}, Barcode field visible: {IsBarcodeFieldVisible}");
                });
        ...
        private void OnApplicationModeChanged(ApplicationModeChangedEvent evt)
        {
            ...
            System.Windows.Application.Current.Dispatcher.InvokeAsync(() =>
            {
                ...
                // Set barcode field visibility based on restaurant mode
                IsBarcodeFieldVisible = evt.IsRestaurantMode;  // CORRECTED: Only visible when IN restaurant mode
        ...
        private async void ToggleView()
        {
            await ExecuteOperationSafelyAsync(async () =>
            {
                ...
                // Ensure barcode field is only visible in restaurant mode
                IsBarcodeFieldVisible = IsRestaurantMode;

                Debug.WriteLine($"Toggled view. IsProductCardsVisible: {IsProductCardsVisible}, IsRestaurantMode: {IsRestaurantMode}, IsBarcodeFieldVisible: {IsBarcodeFieldVisible}");
            ...
            await ShowLoadingAsync("Processing debt transaction...", async () =>
            {
                    ...
                    (SelectedCustomer.Balance + TotalAmount) > SelectedCustomer.CreditLimit)
                {
                    throw new InvalidOperationException(
                        $"Adding this debt would exceed the customer's credit limit of {SelectedCustomer.CreditLimit:N}");
                }
