    ...
    public partial class DrawerViewModel
    {
        ...
        private FlowDocument CreateDrawerReport()
        {
            ...
            if (CurrentDrawer != null)
            {
                ...
                paragraph.Inlines.Add(new Run($"Opening Balance: {CurrentDrawer.OpeningBalance:N}\n"));
                paragraph.Inlines.Add(new Run($"Current Balance: {CurrentDrawer.CurrentBalance:N}\n"));
                paragraph.Inlines.Add(new Run($"Cash In: {CurrentDrawer.CashIn:N}\n"));
                paragraph.Inlines.Add(new Run($"Cash Out: {CurrentDrawer.CashOut:N}\n"));
                paragraph.Inlines.Add(new Run($"Difference: {CurrentDrawer.Difference:N}\n\n"));
            }
            ...
            if (DrawerHistory.Any())
            {
                ...
                foreach (var transaction in DrawerHistory)
                {
                    ...
                    row.Cells.Add(new TableCell(new Paragraph(new Run(transaction.Amount.ToString("N")))));
                    row.Cells.Add(new TableCell(new Paragraph(new Run(transaction.Balance.ToString("N")))));
