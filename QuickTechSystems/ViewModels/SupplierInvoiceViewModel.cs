        ...
        private async Task SettleInvoiceAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Create and show window using try-catch to isolate any issues
                    try
                    {
                        Debug.WriteLine("Creating payment window");

                        // Create the window
                        var paymentWindow = new QuickTechSystems.WPF.Views.SettlePaymentSupplierWindow(
                        ...
                        if (result == true)
                        {
                            ...
                            if (SelectedInvoice != null)
                            {
                                var refreshedInvoice = await _supplierInvoiceService.GetByIdAsync(SelectedInvoice.SupplierInvoiceId);
