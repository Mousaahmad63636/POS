            ...
            try
            {
                ...
                if (!hasClosingTransaction)
                {
                    var closingTransaction = new DrawerTransaction
                    {
                        ...
                        Description = $"Drawer closed by {drawer.CashierName} with final balance of {finalBalance:N}",
                        ActionType = "Close",
                        PaymentMethod = "Cash"
                    };
                ...
                var description = $"Drawer closed by {drawer.CashierName} with {(difference >= 0 ? "surplus" : "shortage")} of {Math.Abs(difference):N}";
        ...
        public async Task<bool> VerifyDrawerBalanceAsync(int drawerId)
        {
            ...
            if (Math.Abs(difference) > 0.01m)
            {
                await LogDrawerAuditAsync(drawerId, "Balance Verification",
                    $"Balance discrepancy detected: {difference:N}");
                return false;
            }
